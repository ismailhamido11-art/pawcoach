...
export function buildRecommendations({ records = [], exercises = [], scans = [], checkins = [], dailyLogs = [], todayCheckin, streak, diagnosisReports = [], nutritionPlans = [] }) {
  ...
  if (overdueVaccine) {
    const vKey = matchVaccineKey(overdueVaccine.title);
    recs.push({
      id: "vaccine_overdue",
      priority: 1,
      icon: Syringe,
  ...
  if (!overdueVaccine) {
    ...
    if (soonVaccine) {
      const vKey = matchVaccineKey(soonVaccine.title);
      recs.push({
        id: "vaccine_soon",
        priority: 2,
        icon: Syringe,
  ...
  if (diagnosisReports.length > 0) {
    ...
    if (daysSince != null && daysSince <= 14) {
      ...
      recs.push({
        ...
        iconColor: isUrgent ? PALETTE.red500 : PALETTE.blue,
  ...
  if (activeMeds.length > 0) {
    ...
    recs.push({
      ...
      iconBg: "bg-emerald-50",
      iconColor: PALETTE.emerald,
      label: `Médicament en cours : ${med.title || "Traitement"}`,
  ...
  if (recentCaution) {
    ...
    if (daysSince <= 7) {
      recs.push({
        ...
        iconColor: recentCaution.verdict === "toxic" ? PALETTE.red500 : PALETTE.amber400,
