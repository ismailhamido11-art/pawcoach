        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1 mt-1.5">
            {sharedSections.slice(0, 4).map(s => (
              <Badge key={s} variant="outline" className="text-[11px] px-1.5 py-0">{s}</Badge>
            ))}
            {sharedSections.length > 4 && (
              <Badge variant="outline" className="text-[11px] px-1.5 py-0">+{sharedSections.length - 4}</Badge>
            )}
          </div>
        </div>
