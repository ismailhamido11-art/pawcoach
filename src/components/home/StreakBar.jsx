...
import { Flame, Trophy, Footprints, Sparkles, Zap, Crown, GraduationCap, PawPrint, Calendar } from "lucide-react";
...
function getNextBadge({ streak, exercises, dailyLogs }) {
  ...
  if (currentStreak < 3) candidates.push({ Icon: Flame, color: "text-orange-500", name: "En forme", current: currentStreak, target: 3 });
  else if (currentStreak < 7) candidates.push({ Icon: Zap, color: "text-amber-500", name: "Habitude", current: currentStreak, target: 7 });
  else if (currentStreak < 30) candidates.push({ Icon: Crown, color: "text-amber-600", name: "Légende", current: currentStreak, target: 30 });

  if (completedExercises < 1) candidates.push({ Icon: Sparkles, color: "text-violet-500", name: "Coach débutant", current: 0, target: 1 });
  else if (completedExercises < 3) candidates.push({ Icon: GraduationCap, color: "text-violet-600", name: "Coach expert", current: completedExercises, target: 3 });

  if (walkDays < 1) candidates.push({ Icon: PawPrint, color: "text-emerald-500", name: "1re balade", current: 0, target: 1 });
  else if (walkDays < 7) candidates.push({ Icon: Calendar, color: "text-blue-500", name: "Régulier", current: walkDays, target: 7 });
    ...
    <div className="mx-4 space-y-2">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                ...
                {nextLevel && (
                  <span className="text-[11px] text-muted-foreground">{nextLevel.label} dans {nextLevel.min - current}j</span>
                )}
              </div>
            ...
            {current >= longest && longest > 1 && (
              <div className="flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                ...
                <span className="text-[11px] font-bold text-emerald-600">Record</span>
              </div>
            )}
          </div>
        </motion.div>
      ...
      {nextBadge && (
        <Link to={createPageUrl("Profile")}>
          ...
          >
            {nextBadge.Icon && <nextBadge.Icon className={`w-5 h-5 ${nextBadge.color}`} />}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-amber-600 font-bold">{nextBadge.current}/{nextBadge.target}</span>
            </div>
          </motion.div>
        </Link>
      )}
