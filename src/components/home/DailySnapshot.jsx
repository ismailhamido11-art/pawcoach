...
function WeightCard({ records, dailyLogs }) {
  ...
  const trendColor = trend === "up" ? "#f59e0b" : trend === "down" ? "#10b981" : "#94a3b8";
...
function MoodCard({ checkins }) {
  ...
  const color = avg >= 3.5 ? "#10b981" : avg >= 2.5 ? "#f59e0b" : avg ? "#ef4444" : "#94a3b8";
...
function TrainingCard({ exercises }) {
  ...
  const color = pct >= 70 ? "#10b981" : pct >= 30 ? "#f59e0b" : "#3b82f6";
  ...
  return (
    <Link to={createPageUrl("Notebook")} className="block">
      <div className="bg-white rounded-2xl p-4 border border-border/30 shadow-sm h-full">
        ...
        {next ? (
          <>
            <p className="text-2xl font-black leading-none" style={{ color: urgent ? "#f59e0b" : "#10b981" }}>
